...
def test_valid_keys(key):
    """Black-box test: validate should succeed for all valid key lengths."""
...
def test_given_verifying_an_invalid_length_key_then_ipe_raised():
    with pytest.raises(
        ...
        Encrypt().validate(params={"key": b"key"})


def test_given_verifying_an_invalid_length_bytes_key_then_ipe_raised():
    # Just pass a short/invalid bytes key; no need to mock anything
    with pytest.raises(
        InvalidParamError,
        match="Invalid input, key must be of length 128, 192 or 256 bits",
    ):
        Encrypt().validate(params={"key": b"shortkey"})


def test_operator_name():
    """Test that Encrypt.operator_name returns 'encrypt'."""
    assert Encrypt().operator_name() == "encrypt"


def test_operator_type():
    """Test that Encrypt.operator_type returns OperatorType.Anonymize."""
